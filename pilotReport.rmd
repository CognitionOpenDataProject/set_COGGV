...
```{r}
#Error Rate
means_body_up <- error_analysis %>%
  filter(body == "U") %>% #only want upright bodies
  filter(!is.na(accuracy)) %>%
  group_by(id, gravity) %>%
  summarise(bu_accu_mean = mean(accuracy)) %>%
  rename(idd = id,
         gravityy = gravity)

means_body_down <- error_analysis %>%
  filter(body == "D") %>% #only want upside-down bodies
  filter(!is.na(accuracy)) %>%
  group_by(id, gravity) %>%
  summarise(bd_accu_mean = mean(accuracy))

means_bodyud <- bind_cols(means_body_up, means_body_down) %>%
  select(id, bu_accu_mean, bd_accu_mean, gravity) %>%
  mutate

means_bodyud_ <- means_bodyud %>%
    rename(bu = bu_accu_mean,
         bd = bd_accu_mean)%>%
  gather(position, errormean, bu, bd) %>%
  select(id, position, errormean, gravity)
